...
function changeTabElementsIncludedResult(clicked, result_id) {
  ...
  $(`div.tab-content .tab-pane[data-result="${result_id}"]`).removeClass(
    'show active'
  );

  // Sadece belirli result_id'ye sahip olan tab ve buttonları aktif hale getir
  $(`div.nav-tabs button.${clicked}[data-result="${result_id}"]`).addClass(
    'active'
  );
  $(
    `div.tab-content .tab-pane.${clicked}[data-result="${result_id}"]`
...
function sendAjaxRequestOLD(
  ...
  xhr.timeout = 10000; // Maksimum 10 saniye beklesin
...
function handleRetry(url, method, data, callback, retry_count, errorMsg) {
  console.error(errorMsg);

  if (retry_count < 3) {
    console.warn(
      `Bağlantı xətası! ${retry_count + 1}. dəfə yenidən cəhd edilir...`
    );
    sendAjaxRequestOLD(url, method, data, callback, retry_count + 1);

  } else {
    console.error('Bağlantı xətası! Maksimum deneme sayısına ulaşıldı.');
    callback &&
      callback(
        new Error('Bağlantı xətası! Maksimum deneme sayısına ulaşıldı.')
      );
  }
}
