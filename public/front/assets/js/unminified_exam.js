...
async function updateClock() {
  // Increment counters only once per second
  sec++;
  question_time_reply++;
  time_exam.value = sec;
  ...
  // Update section timer if active
  if (loader_for_sections && 
      loader_for_sections.classList.contains('active') &&
      section_start_time.value > 0) {
    
    qalan_vaxt = calculateRemainingTime(
      section_start_time.value,
      time_range_sections.value,
      time_exam.value
    );
    
    let minutesQalanVaxt = qalan_vaxt > 0 ? Math.floor(qalan_vaxt / 60) : 0;
    let secondsQalanVaxt = qalan_vaxt > 0 ? qalan_vaxt % 60 : 0;
    
    // Update UI if not on result page
    var page_type = document.getElementById('page_type');
    if (!page_type || page_type.value !== 'result') {
      if (document.getElementById('seconds_start_time')) {
        document.getElementById('seconds_start_time').innerHTML = pad_new(secondsQalanVaxt);
      }
      
      if (document.getElementById('minutes_start_time')) {
        document.getElementById('minutes_start_time').innerHTML = pad_new(minutesQalanVaxt);
      }
  ...
  // Update current question timer
  settimerforcurrentquestion();
  
  // Do NOT call requestAnimationFrame here - let the interval handle timing
}
...
function settimerforcurrentquestion() {
  if (document.getElementById('current_question')) {
    ...
    if (replyElem) {
      ...
      // Only increment by 1 each time this is called
...
function mark_unmark_question (id) {
  ...
  const exam_result_id = document.getElementById ('exam_result_id').value;
  const user_id = document.getElementById ('user_id').value;
  const language = document.getElementById ('language').value;

  let element = document.getElementById (`mark_question_button_${id}`);
...
function toggleabcline(question_id, value_id) {
  // Cache selectors instead of finding them repeatedly
  ...
  const undoRedoElement = document.getElementById(
    `question_container_undo_or_redo_${question_id}_${value_id}`
  );
  
  // Handle radio toggling
  if (radioElement) {
    const hasRemovableClass = radioElement.classList.contains('removable');
    radioElement.classList.toggle('removable');
    undoRedoElement.innerHTML = hasRemovableClass
      ? `<img src="${aplus_icon_path}" class="img-fluid img-responsive" />`
      : `<span>${undo_text}</span>`;
  }
  
  // Handle checkbox toggling
  if (checkboxElement) {
    const hasRemovableClass = checkboxElement.classList.contains('removable');
    checkboxElement.classList.toggle('removable');
    undoRedoElement.innerHTML = hasRemovableClass
      ? `<img src="${aplus_icon_path}" class="img-fluid img-responsive" />`
      : `<span>${undo_text}</span>`;
  }
...
function remove_button_toggler() {
  // Convert HTMLCollection to Array for better performance when manipulating
  const removeButtons = Array.from(document.getElementsByClassName('remove_button'));
  const undoRedoButtons = Array.from(document.getElementsByClassName('btn-question_container_undo_or_redo'));
  const answerContainers = Array.from(document.getElementsByClassName('question_answer_one'));
  
  // Check if buttons should be shown or hidden
  const shouldShow = !undoRedoButtons[0]?.classList.contains('show');
  
  // Process all buttons in batch instead of looping multiple times
  undoRedoButtons.forEach(button => {
    button.classList.toggle('show', shouldShow);
  });
  
  // Only process answer containers and reset icon if we're hiding the buttons
  if (!shouldShow) {
    ...
    undoRedoButtons.forEach(button => {
      button.innerHTML = `<img src="${aplus_icon_path}" class="img-fluid img-responsive" />`;
    });
  }
  
  // Toggle the 'active' class on all remove buttons
