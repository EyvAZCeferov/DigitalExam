...
const countDownDate = new Date(Date.now() + time_end_exam.value * 60 * 1000).getTime();
let erroroccurs = false;
let time_range_sections = document.getElementById('time_range_sections');
let updateInterval;
let errorInterval;
let errorCount = 0;
let sonekrandakivaxt = time_range_sections.value;
let next_section = document.getElementById('next_section');
let section_start_time = document.getElementById('section_start_time');
let time_exam = document.getElementById('time_exam')
    ...
    if (all_questions == currentDivQuestion.dataset.key || tolast == true) {
      if (finishmodalshowed == false && tolast == false) {
        ...
        if (
          ...
          sonekrandakivaxt = sonekrandakivaxt - 1;
...
async function updateClock() {
  ...
  var loader_for_sections = document.getElementById('loader_for_sections');
  let now = new Date();
  let difference = countDownDate - now.getTime();
  let minutesDifference = Math.floor(difference / (1000 * 60));
  let secondsDifference = Math.floor(difference % (1000 * 60) / 1000);
  time_exam.value = sec || 0;
  if (
    loader_for_sections.classList.contains('active') &&
    section_start_time.value > 0
  ) {
    qalan_vaxt =
      parseInt(section_start_time.value) +
      parseInt(time_range_sections.value) -
      parseInt(time_exam.value);
    let minutesQalanVaxtDifference = qalan_vaxt > 0
      ? Math.floor(qalan_vaxt / 60)
      : 0;
  ...
  if (difference <= 0 && erroroccurs) {
    errorCount++;
    if (errorCount >= 3) {
      clearInterval(errorInterval);
      window.location.href = redirect_url || '/error-page';
      return;
    }
    await tonext(true, 'imtahanzamanibitdi');
  }
...
function settimerforcurrentquestion() {
  if (document.getElementById('current_question')) {
    var current_question = document.getElementById('current_question').value;
    if (document.getElementById(`question_time_replies_${current_question}`)) {
      var question_time_replies_currentval =
        document.getElementById(`question_time_replies_${current_question}`)
          .value || 0;
      document.getElementById(
        `question_time_replies_${current_question}`
      ).value =
        parseInt(question_time_replies_currentval) + 1;
    }
  }
}
